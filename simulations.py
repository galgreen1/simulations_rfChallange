...
if __name__ == '__main__':
    ...
    # sigma = math.sqrt(1 / 20)
    # a1, b1 = simulation(sigma,50)
    # sigma = math.sqrt(1 / 10)
    # z1, w1 = simulation(sigma,50)
    # sigma = math.sqrt(1 / 10)
    # c1, d1 = simulation(sigma, 1)
    # ps_ratios, ps_ratios_db = calculate_ps_ratios_db()
    ...
    # # בונים מערך של שורות (len = len(ps_ratios_db)) ו־5 עמודות
    # data_matrix = np.vstack([
    #     ps_ratios_db,  # עמודה 0
    #     np.array(x1),  # עמודה 1: BER ל־SNR=30dB, M=50
    #     np.array(a1),  # עמודה 2: BER ל־SNR=20dB, M=50
    #     np.array(z1),  # עמודה 3: BER ל־SNR=10dB, M=50
    #     np.array(c1)   # עמודה 4: BER ל־SNR=10dB, M=1
    # ]).T  # טרנספוז כדי לקבל צרת (30, 5)

    # header_line = "ps_db\tBER_30dB_M50\tBER_20dB_M50\tBER_10dB_M50\tBER_10dB_M1"
    # np.savetxt("outputs/outputs1.txt", data_matrix, fmt="%.6e", header=header_line, delimiter="\t", comments="")
    # s=[0,2,3,4,5,6]
    # s_es=[0.5,2,3,4,10,11]
    # print(computeCloset_sense(s_es,quan(s)))

    # ps_ratio=1
    # S = create_fmcw()
    # com = create_communication_symbol()
    # # signal_sim = add_noise(ps_ratio, sigma, S, com)
    # # signal_sim =combine_sensing_communication(ps_ratio,S,com)
    # signal_sim = add_CP(ps_ratio, S, com)
    # # for i in range(N):
    # #     print(f"{i} {signal_sim[i]}")
    # #print(signal_sim)
    # # return to 64
    # addedMat = np.zeros((N, N), dtype=complex)
    # for i in range(N):
    ...
    # # es_sense = estimate_sense(ps_ratio, addedMat)
    # # es_com = estimate_com(ps_ratio, addedMat, es_sense)
    # # es_sense = estimate_sense(ps_ratio, signal)
    # es_com = estimate_com(ps_ratio, addedMat, quan(S))
    # closest_symbols = computeCloset(es_com)
    #
    # # print(closest_symbols)
    # # print(com)
    # check_com = closest_symbols - com
    # # print(check_com)
    # # check_sense = es_sense - S
    # # print(check_com)
    # # print(check_sense)
    # # sum_sen = 0
    # sum_com = 0
    # for i in range(N):
    #     # if np.abs(check_sense[i]) > 1e-3:  # in case of little mistakes
    #     # if np.abs(check_sense[i])> 3.7:  # in case of little mistakes
    #     # sum_sen = sum_sen + 1
    #     for j in range(N):
    #         if np.abs(check_com[i][j]) != 0:
    #             # print(f"orig:{com[i][j]} and {closest_symbols[i][j]}")
    ...
    # #     for j in range(N):
    # #         if j < N - CP:
    # #             addedMat[i][j] = signal_sim[i][j + CP]
    # #         else:
    # #             addedMat[i][j] = signal_sim[i][j] + signal_sim[i][j - N + CP]
    #
    # #es_sense = estimate_sense(ps_ratio, addedMat)
    # # es_com = estimate_com(ps_ratio, addedMat, es_sense)
    # # es_sense = estimate_sense(ps_ratio, signal)
    # es_com = estimate_com(ps_ratio, signal_sim, quan(S))
    # closest_symbols = computeCloset(es_com)
    #
    # # print(closest_symbols)
    # # print(com)
    ...
    #a=np.array([[complex(0,0),complex(1,-0.5)],[complex(1,-1),complex(-1,-1)]])
    #print(computeCloset(a))


   #  s=create_communication_symbol()
   #  s2=create_fmcw()
   #  ps_ratio=math.pow(5,-1)
   #  #ps_ratio=0
   #  signal=add_noise(ps_ratio,0.00001,s2,s)
   #  #signal=add_CP(ps_ratio,s2,s)
   #  #signal=combine_sensing_communication(ps_ratio,s2,s)
   #  #return to 64
   #  addedMat = np.zeros((N, N), dtype=complex)
   #  for i in range(N):
   #      for j in range(N):
   #          if j < N - CP:
   #              addedMat[i][j] = signal[i][j + CP]
   #          else:
   #              addedMat[i][j] = signal[i][j] + signal[i][j - N + CP]
   #  es_sense=estimate_sense(ps_ratio, addedMat)
   #  a=estimate_com(ps_ratio,addedMat,es_sense)
   #  #es_sense=estimate_sense(ps_ratio, signal)
   #  #answersense=quan(s2)-es_sense
   #  a=computeCloset(a)
   #  answercom=s-a
   #  print(f"answercom{answercom}")
   #  sumc=0
   #  #sums=0
   #
   # # print(f"answersense{answersense}")
   #  for i in range(N):
   #      #if np.abs(answersense[i])>5:
   #          #sums=sums+1
   #      for j in range(N):
   #          if np.abs(answercom[i][i])!= 0:
   #              sumc=sumc+1
   #
   #  #sums=sums/N
   #  sumc=sumc/(N**2)
   #  #print(f"error bit rate sense:{sums}")
   #  print(f"error bit rate com:{sumc}")
